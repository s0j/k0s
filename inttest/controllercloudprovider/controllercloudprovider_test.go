...
func (s *ControllerCloudProviderSuite) TestK0sGetsUp() {
	s.Require().NoError(s.InitController(0, "--enable-controller-cloud-provider", "--controller-cloud-provider-update-frequency=5s"))
	...
	s.Require().NoError(err)
	s.Require().NotContains(w0labels, controllercloudprovider.ExternalIPLabel)
	...
	w0, err := s.AddNodeLabel(s.WorkerNode(0), kc, controllercloudprovider.ExternalIPLabel, "1.2.3.4")
	s.Require().NoError(err)
	s.Require().NotNil(w0)

	// The controller-cloud-provider is configured to update every 5s, so wait
	// for 10s and then look for the external IP.
	s.T().Logf("waiting 10s for the next controller-cloud-provider update")
	time.Sleep(10 * time.Second)

	// Need to ensure that an 'ExternalIP' address of '1.2.3.4' has been added,
	// indicating that controller-cloud-provider properly processed the label.
...
func TestControllerCloudProviderSuite(t *testing.T) {
	suite.Run(t, &ControllerCloudProviderSuite{
		common.FootlooseSuite{
			ControllerCount: 1,
			WorkerCount:     2,
		},
